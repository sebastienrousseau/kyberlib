...
int main()
{
  ...
  // Create the generate_key_pair file
  sprintf(fn_ckk, "generate_key_pair");
  ...
  for (int i=0; i<100; i++) {
    ...
    randombytes(indcpa, KYBER_SYM_BYTES);
    randombytes(ckk, KYBER_SYM_BYTES);
    randombytes(enc, KYBER_SYM_BYTES);
    fprintBstr(fp_indcpa, indcpa, KYBER_SYM_BYTES);
    fprintBstr(fp_ckk, ckk, KYBER_SYM_BYTES);
    fprintBstr(fp_enc, enc, KYBER_SYM_BYTES);
  }
